...
is the query named "GetEra",
which can be accessed via the [ExampleConnector.getEra] property.


An example of the property for a mutation
is the mutation named "CreateBookmark",
which can be accessed via the [ExampleConnector.createBookmark] property.
...
For example, the "GetMyBookmarks" query has no variables
and can be executed via the
[ExampleConnector.getMyBookmarks]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val queryResult = connector.getMyBookmarks.execute()
println("GetMyBookmarks query returned: ${queryResult.data}")
```
...
For example, the "GetEra" query has 1 required variable ("id")
and can be executed via the [ExampleConnector.getEra]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val queryResult = connector.getEra.execute(id=java.util.UUID.randomUUID())
println("GetEra query returned: ${queryResult.data}")
```
...
For example, the "CreateEra" mutation has no variables
and can be executed via the
[ExampleConnector.createEra]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.createEra.execute()
println("CreateEra mutation returned: ${mutationResult.data}")
```
...
For example, the "CreateGarment" mutation has 4 required variables ("name", "description", "historicalSignificance", and "eraId")
and can be executed via the [ExampleConnector.createGarment]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.createGarment.execute(name="waldo", description="baz", historicalSignificance="grault", eraId=java.util.UUID.randomUUID())
println("CreateGarment mutation returned: ${mutationResult.data}")
```
